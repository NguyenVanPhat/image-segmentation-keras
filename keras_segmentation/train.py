...
def train(model,
          ...
          preprocessing=None,
          read_image_type=1, # cv2.IMREAD_COLOR = 1 (rgb), cv2.IMREAD_GRAYSCALE = 0, cv2.IMREAD_UNCHANGED = -1 (4 channels like RGBA)
          history_logger_path=None
         ):
    ...
    if callbacks is None and (not checkpoints_path is  None) :
        ...
        if history_logger_path is not None and len(history_logger_path) > 0:
            history_logger = tf.keras.callbacks.CSVLogger(history_logger_path, separator=",", append=True)
